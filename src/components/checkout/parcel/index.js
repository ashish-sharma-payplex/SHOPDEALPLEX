...
// import ParcelDeliveryCharges from "./ParcelDeliveryCharges";
...
const ParcelCheckout = () => {
  ...
  // Auto scroll to top when parcel checkout renders
  useEffect(() => {
    // Immediate scroll without smooth behavior for instant effect
    window.scrollTo(0, 0);
    document.documentElement.scrollTo(0, 0);
    document.body.scrollTo(0, 0);

    // Additional scroll attempts with slight delays
    setTimeout(() => {
      window.scrollTo(0, 0);
      document.documentElement.scrollTo(0, 0);
      document.body.scrollTo(0, 0);
    ...
    // Force hide footer by temporarily adjusting its position
    ...
    if (footer) {
      ...
      // Restore footer position after scroll completes
      setTimeout(() => {
    ...
    // Additional footer management with more aggressive approach
    setTimeout(() => {
  ...
  // Function to scroll to top when confirm is clicked
  const scrollToTop = () => {
    // Immediate scroll without smooth behavior for instant effect
    window.scrollTo(0, 0);
    document.documentElement.scrollTo(0, 0);
  ...
  const orderPlace = () => {
    if (paidBy === "sender") {
      const handleSuccess = (res) => {
        if (res) {
          // Handle digital payment (redirect to payment page)
          if (
            paymentMethod !== "cash_on_delivery" &&
            paymentMethod !== "offline_payment" &&
            paymentMethod !== "" &&
      ...
      orderMutation(orderMutationObject, {
        ...
        onError: (error) => {
          ...
          // Only show error messages, don't redirect to checkoutfailed
          // This allows users to retry with different payment methods
        },
      });
    } else {
      ...
      if (paymentMethod === "cash_on_delivery") {
        const handleSuccess = (res) => {
          if (res) {
            // Handle digital payment for receiver pays
            if (
              paymentMethod !== "cash_on_delivery" &&
              paymentMethod !== "offline_payment" &&
              paymentMethod !== "" &&
        ...
        >
          ...
          {sideDrawerOpen && (
            <TrackParcelOrderDrawer
              ...
              setSideDrawerOpen={handleCloseSideDrawer}
            />
          )}
        </CustomStackFullWidth>
      )}
