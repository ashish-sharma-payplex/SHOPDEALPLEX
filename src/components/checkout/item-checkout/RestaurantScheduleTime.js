...
const RestaurantScheduleTime = (props) => {
	...
	const formatTo12HourRange = (timeRange) => {
	...
	return `${convert(startRaw)} - ${convert(endRaw)}`;
};

// 	console.log("STORE SCHEDULES (backend se):", storeData?.schedules);
// 	console.log(
// 	"GENERATED SLOTS:",
	...
	return (
		<>
			{storeData?.schedule_order && (
				<CustomStackFullWidth sx={{ height: "100%", pb: 1.5 }}>
										...
										storeData?.schedules?.length > 0 && (
											<PreferableTimeInput
												...
												options={getAllSchedule(
													...
													...item,
													label: formatTo12HourRange(item.label),
												}))}
												onChange={(e, option) =>
