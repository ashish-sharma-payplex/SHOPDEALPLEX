...
// Only India addresses are allowed for pickup/drop.
          ...
          onClick={() => {
            ...
            geocoder.geocode({ location: tempCenter }, (results, status) => {
              if (status === "OK" && results[0]) {
                // ✅ Reject the pin if it isn't inside India
                if (!isIndianAddressComponents(results[0].address_components)) {
                  toast.error("Please select a location within India");
                  // revert pin back to the last valid location
                  setTempCenter(centerPosition);
                  return;
                }
