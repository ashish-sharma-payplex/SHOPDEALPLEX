...
function NumberBadge({ num, isDark }) {
  return (
    <Box
      sx={{
        width: 26,
...
function FeatureItem({ feature, isDark }) {
  return (
    <Box sx={{ width: "100%", boxSizing: "border-box" }}>
      <Box
        sx={{ display: "flex", alignItems: "flex-start", gap: 1.2, mb: 0.75 }}
      >
        <NumberBadge num={feature.id} isDark={isDark} />
        <Typography
          sx={{
            fontFamily: "Inter",
            fontWeight: 500,
            color: isDark ? "#e8eaec" : "#181818",
          ...
          >
            ...
            >
              {features.map((f) => (
                ...
                >
                  <FeatureItem feature={f} isDark={isDark} />
                </Box>
              ))}
            </Box>
          </Box>
