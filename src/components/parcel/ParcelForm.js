...
const ParcelForm = ({ onNext, parcelData }) => {
  ...
  const isDark = theme.palette.mode === "dark"; // only flips on the hardcoded dark classes below
  ...
  const [selectedCategoryId, setSelectedCategoryId] = useState(
    initRaw.selectedCategoryId || "",
  );
  const [selectedWeightId, setSelectedWeightId] = useState(
    initRaw.selectedWeightId || "",
  );
  ...
  const getFrontendErrors = () => {
    ...
    if (!selectedCategoryId)
      errors.selectedCategoryId = "Please select parcel category";
    if (!selectedWeightId)
      errors.selectedWeightId = "Please select weight range";
  ...
  const handleConfirm = () => {
    ...
    const selectedWeightObj = STATIC_WEIGHT_RANGES.find(
          ...
          <Grid container spacing={2} alignItems="stretch">
            ...
            <Grid item xs={12} md={5} sx={{ display: "flex" }}>
              <Box
                className={
                  isDark
                    ? `${styles.sectionBoxDark} ${styles.inputsDark}`
                    : undefined
                }
                sx={{
                  ...
                  ...(isDark && { border: "none", bgcolor: "transparent" }),
                }}
              >
                {/* Parcel Category */}
                ...
                <TextField
                  ...
                  SelectProps={{
                    renderValue: (selected) => {
                      ...
                      const item = STATIC_WEIGHT_RANGES.find(
                        (w) => w.id === selected,
                      );
                      return (
                        <span style={{ fontSize: "12px" }}>
                          {item ? item.weight_range : selected}
                        </span>
                      );
                    },
                  }}
                ...
                <Card
                  ...
                  sx={{ p: 2, mb: 2, borderRadius: "8px !important" }}
                >
                  <Box display="flex" justifyContent="space-between">
                    <Box>
                      ...
                      <Typography
                        ...
                        className={
                          isDark ? styles.textSecondaryDark : undefined
                        }
                      >
                        Need Extensive care
                      </Typography>
