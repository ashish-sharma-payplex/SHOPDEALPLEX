...
const ProductCardIncrementDecrement = ({ isInCart, modalData }) => {
  ...
  const getCurrentCartItem = (id) => {
    return cartList.find((cartItem) => cartItem.id === id);
  };

  const handleIncrement = (e) => {
    e.stopPropagation();
    const currentItem = getCurrentCartItem(modalData?.id);
    if (getCurrentModuleType() !== "food") {
      if (modalData?.stock <= getQuantity(modalData?.id)) {
        toast.error(t(out_of_stock));
      } else {
        reduxDispatch(setIncrementToCartItem(currentItem));
      }
    } else {
      reduxDispatch(setIncrementToCartItem(currentItem));
    }
  ...
  const handleDecrement = (e) => {
    e.stopPropagation();
    const currentItem = getCurrentCartItem(modalData?.id);
    reduxDispatch(setDecrementToCartItem(currentItem));
  };
  const handleRemove = (e) => {
    e.stopPropagation();
    const currentItem = getCurrentCartItem(modalData?.id);
    reduxDispatch(setRemoveItemFromCart(currentItem));
  };
  return (
    <>
      <CustomCardButton
