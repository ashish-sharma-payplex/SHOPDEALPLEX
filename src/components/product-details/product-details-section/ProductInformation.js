...
const ProductInformation = ({
	...
	const { data: reviewsData, refetch: refetchReviews } = useGetProductReviews({
	...
	const addToCard = () => {
		...
		// Remove store restriction: always allow adding
		handleAddToCartOnDispatch();
	};
	...
	const topInformation = () => {
		return (
			...
			>
				...
				{state.modalData[0]?.generic_name && state.modalData[0]?.generic_name[0] && (
					<Typography
					...
					<Stack direction="row" alignItems="center" spacing={1}>
						...
						>
							...
							>
								...
								<Typography variant="body1" sx={{ fontWeight: "400", display: 'inline', px:1 }}>
									{t("Reviews")}
								</Typography>
							</Typography>
						</Stack>
	...
	return (
		<>
			...
			<CustomModal openModal={openReviewModal} handleClose={() => setOpenReviewModal(false)}>
				<Box
					sx={{
				...
				>
					...
					<Stack spacing={2}>
						<Typography variant="h6" fontWeight="bold">
							{state.modalData[0]?.name ? `${state.modalData[0]?.name} Reviews` : "Product Reviews"}
						</Typography>
						{reviewsData?.reviews?.length > 0 ? (
							<ProductReviews
								...
								offSet={0}
								total_size={reviewsData.total_size}
								setOffSet={() => { }}
								page_limits={10}
								isExpanded="true"
								storename={state.modalData[0]?.store_name}
							/>
						) : (
