...
const DetailsAndReviews = (props) => {
	...
	const { t } = useTranslation();
	const [tabs, setTabs] = useState(defaultTab === 'reviews' ? 1 : 0);
	...
	useEffect(() => {
		if (contentRef.current) {
			...
			if (contentRef.current.scrollHeight > maxHeightForTwoLines) {
				setShowButton(true); // ✅ 2 line se zyada
			} else {
				setShowButton(false); // ❌ 2 line ya kam
			}
		}
	}, [description]);
	...
	return (
		<Wrapper>
			...
			>
				...
				<CustomHeader info={tabs === 0 ? "Detailed Description" : "Reviews"} />
				<CustomStackFullWidth p="0px 0px">
					{tabs !== 0 && (
						<ProductReviews
							reviews={
								vehicleReview
