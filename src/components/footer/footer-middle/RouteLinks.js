...
const RouteLinks = (props) => {
  ...
  return (
    <CustomStackFullWidth spacing={2} alignItems={{ xs: "start" }}>
      {RouteLinksData.map((item, index) => {
        return (
          ...
          >
           ...
           {/* Quick Help Section - new section */}
      {/* {quickHelpLinks && quickHelpLinks.map((item, index) => (
        <Typography
          key={index}
          onClick={() => handleClickToRoute(item.link)}
          sx={{
            cursor: "pointer",
            "&:hover": {
              color: theme.palette.primary.main,
      ...
      {/* {policiesLinks && policiesLinks.map((item, index) => (
        <Typography
          key={index}
          onClick={() => handleClickToRoute(item.link)}
          sx={{
      ...
      {/* Social Links Section - new section */}
