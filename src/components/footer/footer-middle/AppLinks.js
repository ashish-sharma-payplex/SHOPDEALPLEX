...
const AppLinks = (props) => {
  const { graybackground, landingPageData, size="small" } = props;
  ...
  const getButtonSize = () => {
    if (size === "small") {
      return { height: "40px", fontSize: "0.75rem" }; // Small button size
    }
    return { height: "50px", fontSize: "1rem" }; // Default medium button size
  };
  const googlePlay = () => (
    <CustomButton
      onClick={() =>
