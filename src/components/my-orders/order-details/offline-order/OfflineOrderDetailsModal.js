  ...
  return (
    ...
    >
      {page === "my-orders?flag=fail" ? (
        ...
        >
          {t("Order Cancelled due to Payment Failure")}
        </Typography>
      ) : (
        <>
          <CheckCircleIcon
          ...
          <Typography fontSize="14px" fontWeight="400">
            {page === "my-orders?flag=cancel" || page === "my-orders?flag=fail" ? (
              <Typography color={theme.palette.error.main}>
                {t("Your order has been cancelled due to payment failure.")}
              </Typography>
            ) : (
              `${t("Your Item has been successfully processed, and your order ")} !`
            )}
