...
const TopDetails = (props) => {
  const {
    ...
    dataIsLoading,
    page,
    driverLocation, // 👈 socket se aaya hua driver location data
  } = props;
  const { t } = useTranslation();
  const theme = useTheme();

  // console.log("Order data from TopDetails page :", data)
 ...
 const buttonBackgroundColor = () => {
  ...
  if (
    ...
    trackData?.order_status === "arrived_at_pickup" ||
    trackData?.order_status === "drop_arrived" ||      // 👈 add
    trackData?.order_status === "drop_verified"        // 👈 add
  ) {
    return theme.palette.warning.dark;
  }
  if (
    trackData?.order_status === "delivered" ||
    trackData?.order_status === "completed"            // 👈 add
  ) {
  ...
  if (deliveryTime && processingTime === null) {
    const timeArr = deliveryTime.split("-");
    minTime = Number.parseInt(timeArr[0]);
  }

  const baseTime = scheduleAt || createdAt;

  const newTime = moment(baseTime).add(minTime, "minutes");

  const diff = newTime.diff(moment(), "minutes"); // ✅ correct way
 ...
 const handleTime = () => {
  ...
  if (!diff || diff <= 0) {
    return trackData?.store?.delivery_time || "10-30";
  }
        ...
        {dataIsLoading ? (
          ...
          <Typography fontSize={{ xs: "12px", md: "16px" }} fontWeight="600">
            ...
            <Typography
              ...
              marginLeft="5px"
            >
              {data?.[0]?.order_id || data?.id || trackData?.id || id}
            </Typography>
  ...
  >
    ...
    {trackData?.otp}
        ...
        {/* 👇 Driver live location badge — socket se aane wala data */}
        {driverLocation && (
          <Typography
            fontSize={{ xs: "10px", md: "12px" }}
            fontWeight="500"
          ...
          >
            📍 {t("Lat")}: {driverLocation?.lat} | {t("Lng")}: {driverLocation?.lng}
            {driverLocation?.speed != null && ` | ${t("Speed")}: ${driverLocation.speed}`}
          </Typography>
        )}
      </Stack>
      ...
      {data &&
        ...
        trackData &&
        trackData?.order_status === "deliveredd" &&
