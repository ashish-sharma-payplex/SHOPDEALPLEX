...
const DeliverymanForm = ({ data, orderId }) => {
  ...
  const isAlreadyReviewed = data?.is_reviewed || data?.review_rating;
  const existingRating = data?.review_rating || data?.rating;
  const existingComment = data?.review_comment || data?.comment;
          ...
          <Grid item xs={12} md={12} align="center">
            <Stack alignItems="center">
              <CustomTypographyGray sx={{ fontSize: "18px" }}>
                {isAlreadyReviewed ? t("Your rating") : t("Rate the deliveryman")}
              </CustomTypographyGray>
              <CustomRatings
                handleChangeRatings={handleChangeRatings}
