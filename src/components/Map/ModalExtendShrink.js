...
const ModalExtendShrink = (props) => {
  ...
  return (
    <Wrapper isXSmall={isXSmall} onClick={() => setIsModalExpand((prev) => !prev)}>
      {isModalExpand ? (
        <Tooltip title={t("Close fullscreeeen")} arrow placement="top">
          <FullscreenExitIcon sx={{ fontSize: { xs: "18px", md: "24px",backgroundColor:"#ffffff",border:"none" } }} />
        </Tooltip>
      ) : (
        <Tooltip title={t("Fullscreennn")} arrow placement="top">
          <FullscreenIcon sx={{ fontSize: { xs: "18px", md: "24px",backgroundColor:"#ffffff" } }} />
        </Tooltip>
      )}
    </Wrapper>
  );
