...
const GoogleMapComponent = ({
  ...
  if (loadError) {
    return (
      <div style={{ padding: '20px', textAlign: 'center', color: 'red' }}>
        Error loading Google Maps. Please check your API key and console for technical details.
      </div>
    );
  }
  ...
  if (!process.env.NEXT_PUBLIC_GOOGLE_MAP_KEY) {
    return (
      <div style={{ padding: '20px', textAlign: 'center', color: 'orange' }}>
        Google Maps API key is not configured. Please set NEXT_PUBLIC_GOOGLE_MAP_KEY in your environment variables.
      </div>
    );
  }
  ...
  return isLoaded ? (
    ...
    "&:hover": {
      backgroundColor: alpha("#ffffff", 0.8), // 🔥 hover effect
    },
