...
const ProfileTab = ({
  ...
  // Check if mobile or desktop for dynamic menu
  const isMobile = window.innerWidth <= 768; // Example check, replace with your logic
  const tabMenu = menuData(isMobile)?.filter((item) => item?.id !== 10); // Removing cart menu item (id 10)
    ...
    >
      ...
      >
        {tabMenu?.map((item, index) => {
          // Filter items based on configData
          if (
            ...
            return (
              <Box key={index}>
                ...
                >
                  <Typography
                    fontWeight={item?.name === page.split("?")[0] ? "600" : "400"}
                    color={
