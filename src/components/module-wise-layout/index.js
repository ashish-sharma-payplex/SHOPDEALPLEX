...
const ModuleWiseLayout = ({ configData, landingPageData }) => {
  ...
  // New useEffect to set selectedModule from query param
  useEffect(() => {
    if (data?.length > 0 && router.query.module) {
      const moduleName = router.query.module.toLowerCase();
      const matchedModule = data.find(
        (m) => m.module_name.toLowerCase() === moduleName,
      );
      if (matchedModule) {
        localStorage.setItem("module", JSON.stringify(matchedModule));
        dispatch(setSelectedModule(matchedModule));
      }
    }
  }, [data, router.query.module, dispatch]);
