...
const DEFAULT_TICKET_STATUS_STYLE = {
  label: "Pending",
  subtitle: "Awaiting Confirmation",
  bg: "var(--mb-warn-bg)",
  text: "var(--mb-warn-strong-text)",
  icon: ScheduleIcon,
};
...
const getItinerary = (booking) =>
  booking?.raw?.FlightItinerary || booking?.FlightItinerary || {};
...
const BookingDetailsDialog = ({
  ...
  const journeyList = (bookings || []).filter(
    (b) => getItinerary(b)?.Segments?.length,
  );
  ...
  return (
    ...
    >
      ...
      <DialogContent
        sx={{ p: { xs: 1.25, sm: 3 }, bgcolor: "var(--mb-dialog-bg)" }}
      >
        {journeyList.length === 0 ? (
          ...
          journeyList.map((booking, jIdx) => {
            ...
            const fare = fi.Fare || {};
            const taxBreakup = (fare.TaxBreakup || []).filter(
              (t) => t.key !== "TotalTax",
            );
            ...
            const statusStyle =
              TICKET_STATUS_STYLES[ticketStatus] || DEFAULT_TICKET_STATUS_STYLE;
            const StatusIcon = statusStyle.icon;

            const bookingDate =
              passengers[0]?.Ticket?.IssueDate ||
              fi.Invoice?.[0]?.InvoiceCreatedOn;
            const journeyTypeLabel = isMultiJourney
              ? booking.journey_type || (jIdx === 0 ? "Onward" : "Return")
              : "One Way";
            const ticketTypeLabel =
              passengers[0]?.Ticket?.TicketType === "N"
                ? "E-Ticket"
                : passengers[0]?.Ticket?.TicketType || "—";

            return (
              <Box
                key={booking.booking_id || jIdx}
                sx={{ mb: jIdx < journeyList.length - 1 ? 3 : 0 }}
              >
                {isMultiJourney && (
                  <Chip
                    ...
                    size="small"
                    sx={{
                      bgcolor: "var(--mb-indigo-bg)",
                      color: "var(--mb-indigo-text)",
                      fontWeight: 700,
                      mb: 1.5,
                    }}
                  />
                ...
                <Box sx={cardSx}>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <FlightIcon
                          sx={{ color: GREEN, transform: "rotate(45deg)" }}
                        />
                      </Box>
                      <Box sx={{ minWidth: 0 }}>
                        <Box
                        ...
                        <Typography
                          sx={{ fontSize: 12, color: "var(--mb-text-faint)" }}
                        >
                    ...
                    >
                      <Box sx={{ textAlign: "center" }}>
                        ...
                        <Typography
                          sx={{ fontSize: 11, color: "var(--mb-text-faint)" }}
                        >
                    ...
                    >
                      <StatusIcon
                        sx={{ fontSize: 18, color: statusStyle.text }}
                      />
                      <Box>
                        <Typography
                          sx={{
                  ...
                  >
                    ...
                    <MetaItem
                      label="Fare Type"
                      value={firstSeg?.Remark || fi.FareType || "—"}
                    />
                    <MetaItem label="Ticket Type" value={ticketTypeLabel} />
                    <MetaItem
                      label="Is Domestic"
                      value={
                        fi.IsDomestic === undefined
                          ? "—"
                          : fi.IsDomestic
                          ? "Yes"
                          : "No"
                            ...
                            >
                              ...
                              <Box sx={{ minWidth: 0 }}>
                                ...
                                >
                                  ...
                                  <Chip
                                    label={
                                      PAX_TYPE_LABEL[p.PaxType] || "Passenger"
                                    }
                                    size="small"
                                    sx={{
                  ...
                  {segments.map((seg, idx) => (
                    ...
                    >
                      <Box sx={{ textAlign: { xs: "left", sm: "center" } }}>
                        ...
                        >
                          {seg.Origin?.DepTime
                            ? new Date(seg.Origin.DepTime).getDate()
                            : "—"}
                        </Typography>
                        <Typography
                          sx={{ fontSize: 11, color: "var(--mb-text-muted)" }}
                        >
                          {formatDate(seg.Origin?.DepTime, {
                            month: "short",
                            year: "numeric",
                          })}
                        </Typography>
                      ...
                      <Box>
                        ...
                        <Typography
                          sx={{ fontSize: 11.5, color: "var(--mb-text-faint)" }}
                        >
                      ...
                      <Box sx={{ textAlign: "center", px: { xs: 0, sm: 2 } }}>
                        ...
                        >
                          <Box
                            sx={{
                              width: { xs: 24, sm: 40 },
                              borderTop: "1px dashed var(--mb-border-strong)",
                            }}
                          />
                          <FlightIcon
                            sx={{
                      ...
                      <Box
                        sx={{
                          borderLeft: {
                            xs: "none",
                            sm: "1px solid var(--mb-border-soft)",
                          },
                          borderTop: {
                            xs: "1px solid var(--mb-border-soft)",
                            sm: "none",
                          },
                          pl: { xs: 0, sm: 2 },
                          pt: { xs: 1.5, sm: 0 },
                      ...
                      >
                        ...
                        <Typography
                          sx={{ fontSize: 11.5, color: "var(--mb-text-faint)" }}
                        >
                          {CABIN_CLASS_LABEL[seg.CabinClass] || "Economy"}{" "}
                ...
                >
                  <Box sx={cardSx}>
                    ...
                    <FareRow
                      label="Other Charges"
                      value={money(fare.OtherCharges)}
                    />

                    <Divider sx={{ my: 1.5 }} />

                    <FareRow
                      label="Total Baggage Charges"
                      value={money(fare.TotalBaggageCharges)}
                    />
                    <FareRow
                      label="Total Meal Charges"
                      value={money(fare.TotalMealCharges)}
                    />
                    <FareRow
                      label="Total Seat Charges"
                      value={money(fare.TotalSeatCharges)}
                    />
                    <FareRow
                      label="Total Special Service Charges"
                      value={money(fare.TotalSpecialServiceCharges)}
                    />
                    ...
                    >
                      <InfoOutlinedIcon
                        sx={{ fontSize: 14, color: "var(--mb-text-faint)" }}
                      />
                      <Typography
                        sx={{ fontSize: 11.5, color: "var(--mb-text-faint)" }}
                      >
                        Published Fare: {money(fare.PublishedFare)}{" "}
                ...
                <Box sx={cardSx}>
                  ...
                  >
                    ...
                    <MetaItem
                      label="Issuance PCC"
                      value={fi.IssuancePcc || "—"}
                    />
                    <MetaItem
                      label="Is Partial Void Allowed"
                      value={fi.IsPartialVoidAllowed ? "Yes" : "No"}
                    />
                    <MetaItem
                      label="Booking Allowed For Roamer"
                      value={fi.BookingAllowedForRoamer ? "Yes" : "No"}
                    />
                    <MetaItem label="PNR" value={fi.PNR || "—"} />
                    <MetaItem label="Is LCC" value={fi.IsLCC ? "Yes" : "No"} />
                    <MetaItem
                      label="Is Auto Reissuance Allowed"
                      value={fi.IsAutoReissuanceAllowed ? "Yes" : "No"}
                    />
                    <MetaItem
                      label="TBO Confirmation No."
                      value={fi.TBOConfNo || "—"}
                    />
                    <MetaItem
                      label="Non Refundable"
                      value={fi.NonRefundable ? "Yes" : "No"}
                    />
                  </Box>
                </Box>
                ...
                >
                  ...
                  <Typography
                    sx={{
                      fontSize: 12.5,
                      color: "var(--mb-indigo-strong-text)",
                    }}
                  >
                    This is an e-ticket. Please carry a valid photo ID at the
                    time of travel.
                  </Typography>
                </Box>

                {isMultiJourney && jIdx < journeyList.length - 1 && (
                  <Divider sx={{ mt: 3 }} />
                )}
              </Box>
            );
...
const FareRow = ({ label, value, onClick, expandable, expanded }) => (
  ...
  >
    <Typography sx={{ fontSize: 13.5, color: "var(--mb-text-body)" }}>
      {label}
    </Typography>
    <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
      <Typography
        sx={{ fontSize: 13.5, fontWeight: 700, color: "var(--mb-text-strong)" }}
      >
        {value}
      </Typography>
      {expandable && (
...
const ExtraTile = ({ icon, label, value, bg, color }) => (
  ...
  >
    ...
    <Box sx={{ minWidth: 0 }}>
      <Typography
        noWrap
        sx={{ fontSize: 11.5, color: "var(--mb-text-muted)", fontWeight: 600 }}
      >
        {label}
      </Typography>
      <Typography
        noWrap
        sx={{ fontSize: 13.5, fontWeight: 800, color: "var(--mb-text-strong)" }}
      >
        {value}
      </Typography>
    </Box>
