...
const LIGHT_BG = "var(--ht-page)";
const CARD_BG = "var(--ht-surface)";
const TEXT_DARK = "var(--ht-text-strong)";
const TEXT_MID = "var(--ht-text-body)";
const TEXT_LIGHT = "var(--ht-text-muted)";
const ERROR_COLOR = "var(--ht-danger-text)";
...
const getInputSx = (hasError) => ({
  "& .MuiOutlinedInput-root": {
    ...
    "& fieldset": {
      borderColor: hasError ? ERROR_COLOR : "var(--ht-border-strong)",
      borderWidth: "1px",
    },

    "&:hover fieldset": {
      borderColor: hasError ? ERROR_COLOR : "var(--ht-text-faint)",
    },
...
const Stars = ({ count = 4 }) =>
  Array.from({ length: 5 }).map((_, i) =>
    i < count ? (
      <StarIcon key={i} sx={{ fontSize: 14, color: "var(--ht-star)" }} />
    ) : (
      <StarBorderIcon
        key={i}
        sx={{ fontSize: 14, color: "var(--ht-text-disabled)" }}
      />
    ),
  );
