                ...
                >
                  {LEFT_COLS.map((col, ci) => {
                    ...
                    const isOtherSelected =
                      !isThisSelected && allSelected.has(seat.Code);
...
export default function SSRSeatPage() {
  ...
  const hasAnyMealOptions = mealSegments.some((seg) =>
    (seg.meals || []).some((m) => m.Code !== "NoMeal"),
  );
  const hasAnyBaggageOptions = baggageSegments.some((seg) =>
    (seg.baggage || []).some((b) => b.Code !== "NoBaggage"),
  );
  ...
  const extraMealTotal = useMemo(() => {
    ...
    Object.values(selectedMeals).forEach((segMap) =>
      Object.values(segMap).forEach((m) => {
        total += m.Price || 0;
      }),
    );
    return total;
  ...
  const extraBaggageTotal = useMemo(() => {
    ...
    Object.values(selectedBaggage).forEach((segMap) =>
      Object.values(segMap).forEach((b) => {
        total += b.Price || 0;
      }),
    );
    return total;
  ...
  const allPassengers = useMemo(() => {
    ...
    (travellers?.infants || []).forEach((t, i) =>
      list.push({ ...t, ptype: "Infant", idx: i }),
    );
    return list;
  }, [travellers]);
  ...
  const seatSegments = useMemo(() => {
    ...
    if (isCombinedRoundTrip) {
      return (onwardSSR?.SeatsBySegment || []).filter(hasRealSeats).map((s) => {
        const leg = classifyCombinedLeg(s.flight_number);
        return {
          ...s,
          leg,
    ...
    const ret = isRoundTrip
      ? (returnSSR?.SeatsBySegment || []).filter(hasRealSeats).map((s) => ({
          ...s,
          leg: "return",
          key: `return-${s.segment_key}`,
          label: `${s.origin} → ${s.destination}`,
  ...
  const seatTotal = useMemo(() => {
    ...
    Object.entries(seatSelections).forEach(([segKey, passMap]) => {
      Object.values(passMap).forEach((seatObj) => {
  ...
  const grandTotal =
    baseFareTotal + extraMealTotal + extraBaggageTotal + seatTotal;
  ...
  const getTravellerMealSummary = (trav) => {
    ...
    if (count === 0)
      return mealSelectionApplicable ? "Meal Required" : "Select Meal";
        ...
        html: `
          <div style="font-size:14px;color:var(--fl-text-body);line-height:1.8;text-align:left">
            <div>
              ...
              <strong>${
                error ||
                "Something went wrong while loading meal, baggage & seat options."
              }</strong>
            </div>
          </div>
        `,
  ...
  if (!loading && error && !onwardSSR) {
    return (
      ...
      >
        ...
        >
          Failed to load meal, baggage &amp; seat options
        </div>
        <div
          style={{ fontSize: 13, color: "var(--fl-text-muted)", maxWidth: 380 }}
        >
          {error}
        </div>
        <button
    ...
    >
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .ssr-header{display:flex;align-items:center;justify-content:space-between;padding:0 20px;border-bottom:1px solid var(--fl-border-soft);gap:12px;flex-wrap:wrap;}
        @media(max-width:768px){.ssr-header{flex-direction:column;align-items:flex-start;padding:0 16px;}}
        .card{background:var(--fl-surface);border-radius:14px;box-shadow:0 1px 10px rgba(0,0,0,0.07);overflow:hidden;min-width:0;}
        ...
        .seg-tabs{display:flex;gap:8px;padding:12px 20px;border-bottom:1px solid var(--fl-border-soft);overflow-x:auto;scrollbar-width:none;}
        .seg-tabs::-webkit-scrollbar{display:none;}
        .seg-btn{border:1.5px solid var(--fl-border);border-radius:8px;background:var(--fl-surface);cursor:pointer;font-family:inherit;font-size:13px;font-weight:500;padding:7px 16px;color:var(--fl-text-body);transition:all 0.15s;display:flex;align-items:center;gap:6px;white-space:nowrap;flex-shrink:0;position:relative;}
        .seg-btn.active{border-color:var(--fl-brand-line);background:var(--fl-success-bg);color:var(--fl-brand-text);font-weight:600;}
        .seg-btn.incomplete::after{content:'';position:absolute;top:-3px;right:-3px;width:8px;height:8px;border-radius:50%;background:#dc2626;border:1.5px solid var(--fl-surface);}
        ...
        .seg-leg-tag.onward{background:var(--fl-info-bg);color:var(--fl-info-text);}
        .seg-leg-tag.return{background:var(--fl-purple-bg);color:var(--fl-purple-text);}
        .meal-card{border:1.5px solid var(--fl-border);border-radius:10px;padding:14px 16px;cursor:pointer;transition:all 0.15s;display:flex;justify-content:space-between;align-items:center;background:var(--fl-surface);min-width:0;gap:10px;}
        .meal-card:hover{border-color:var(--fl-brand-line);background:var(--fl-success-bg);}
        .meal-card.selected{border-color:var(--fl-brand-line);background:var(--fl-success-bg);}
        .bag-card{border:1.5px solid var(--fl-border);border-radius:10px;padding:14px 16px;cursor:pointer;transition:all 0.15s;display:flex;align-items:center;gap:12px;background:var(--fl-surface);min-width:0;}
        .bag-card:hover{border-color:var(--fl-brand-line);background:var(--fl-success-bg);}
        .bag-card.selected{border-color:var(--fl-brand-line);background:var(--fl-success-bg);}
        .add-btn-sm{border:1.5px solid var(--fl-brand-line);border-radius:8px;background:var(--fl-surface);color:var(--fl-brand-text);font-size:13px;font-weight:600;padding:6px 20px;cursor:pointer;font-family:inherit;transition:background 0.15s;white-space:nowrap;flex-shrink:0;}
        .add-btn-sm:hover{background:var(--fl-success-bg);}
        .add-btn-sm.added{background:var(--fl-brand);color:#fff;border-color:var(--fl-brand-line);}
        .trav-chip{border:1.5px solid var(--fl-border);border-radius:999px;background:var(--fl-surface);cursor:pointer;font-family:inherit;padding:8px 14px;text-align:left;transition:all .15s;white-space:nowrap;flex-shrink:0;min-width:max-content;position:relative;}
        .trav-chip.active{border-color:var(--fl-brand-line);background:var(--fl-success-bg);}
        .trav-chip.incomplete::after{content:'';position:absolute;top:-3px;right:-3px;width:8px;height:8px;border-radius:50%;background:#dc2626;border:1.5px solid var(--fl-surface);}
        .fare-row{display:flex;justify-content:space-between;align-items:center;padding:10px 20px;font-size:14px;color:var(--fl-text-body);gap:8px;}
        ...
        .skip-btn{background:none;border:none;cursor:pointer;color:var(--fl-brand-text);font-size:13px;font-weight:600;font-family:inherit;display:flex;align-items:center;gap:4px;white-space:nowrap;flex-shrink:0;}
        .back-btn{background:none;border:none;cursor:pointer;color:var(--fl-text-muted);font-size:13px;font-weight:600;font-family:inherit;display:flex;align-items:center;gap:4px;white-space:nowrap;flex-shrink:0;}
              ...
              {phase === "ssr" ? (
                <>
                  ...
                  <div className="ssr-header">
                    <div style={{ display: "flex", gap: 24, minWidth: 0 }}>
                      <button
                        className={`tab-btn ${
                          activeTab === "meals" ? "tab-active" : "tab-inactive"
                        }`}
                    ...
                    {!isMealRequired && (
                      <button className="skip-btn" onClick={handleSSRSkip}>
                        Skip to {seatApplicable ? "Seat Selection" : "Payment"}{" "}
                        <ChevronRight />
                      </button>
                    )}
                  </div>
                  ...
                  {mealSelectionApplicable && (
                    ...
                    >
                      🍽️ This airline requires a meal selection for every
                      traveller before you can continue.
                    </div>
                  )}
                      ...
                      {availableBaggage.length === 0 ? (
                        ...
                        >
                          {availableBaggage.map((bag) => {
                            ...
                            const isSelected =
                              getSelectedBag(segKey, currentTraveller?.id)
                                ?.Code === bag.Code;
                            return (
                              <div
                                key={bag.Code}
                      ...
                      {allTravellers.map((trav, idx) => {
                        ...
                        return (
                          <button
                            key={trav.id}
                            className={`trav-chip ${
                              activeTravellerIdx === idx ? "active" : ""
                            } ${travIncomplete ? "incomplete" : ""}`}
                ...
                <>
                  ...
                  >
                    ...
                    {!isSeatRequiredFlag && (
                      <button className="skip-btn" onClick={handleSeatsSkip}>
                        Skip to Payment <ChevronRight />
                      </button>
                    )}
                  </div>
                  ...
                  {isSeatRequiredFlag && (
                    ...
                    >
                      💺 This airline requires a seat selection for every
                      traveller before you can continue.
                    </div>
                  )}
                  ...
                  {!seatApplicable ? (
                    ...
                    <>
                      <div className="seg-tabs">
                        {seatSegments.map((seg, idx) => {
                          const segIncomplete =
                            isSeatRequiredFlag &&
                            allPassengers.some(
                              (p) => !(seatSelections[seg.key] || {})[p.id],
                            );
                          return (
                            <button
                              key={seg.key}
                              className={`seg-btn ${
                                activeSeatSegIdx === idx ? "active" : ""
                              } ${segIncomplete ? "incomplete" : ""}`}
                              onClick={() => setActiveSeatSegIdx(idx)}
                            >
                              <div
                                style={{
                                  color:
                                    activeSeatSegIdx === idx
                                      ? "var(--fl-brand-text)"
                                      : "var(--fl-text-faint)",
                                }}
                              >
                      ...
                      >
                        ...
                        >
                          {activeSeatSeg &&
                          Object.keys(seatRowsMap).length > 0 ? (
                            <CabinMap
                              seatRows={seatRowsMap}
                              selections={activeSeatSegSelections}
      ...
      extraBaggageTotal) dikhte hain. */}
                ...
                >
                  <span
                    style={{ fontWeight: 700, color: "var(--fl-text-strong)" }}
                  >
                    {isRoundTrip && !isCombinedRoundTrip
                      ? "Onward Subtotal"
                      : "Subtotal"}
                  ...
                  >
                    ₹
                    {(isCombinedRoundTrip
                      ? onwardFareTotal + extraMealTotal + extraBaggageTotal
                      : onwardSubtotal
                    ).toLocaleString("en-IN")}
                  </span>
                </div>
