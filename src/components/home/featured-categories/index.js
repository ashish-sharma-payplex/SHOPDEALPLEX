...
import { CustomRentalCard } from "../module-wise-components/rental/components/global/CustomRentalCard";
...
const FeaturedCategories = ({ configData }) => {
  ...
  const moduleWiseCard = () => {
    switch (getCurrentModuleType()) {
        ...
        return (
          ...
          >
            ...
            <div style={{ marginTop: "10px", display: "flex", justifyContent: "center", gap: "10px" }}>
              <ButtonLeft onClick={() => slider.current.slickPrev()}>{'<'}</ButtonLeft>
              <ButtonRight onClick={() => slider.current.slickNext()}>{'>'}</ButtonRight>
            </div>
          </CustomBoxFullWidth>
        );
      ...
      case ModuleTypes.RENTAL:
        return (
          <>
            <Slider {...rentalCategorySliderSettings} ref={slider}>
              {featuredCategories?.map((item, index) => {
                return (
                  <CustomRentalCard
                    key={item?.id}
    ...
    switch (getCurrentModuleType()) {
      ...
      case ModuleTypes.RENTAL:
        return (
          <Slider {...rentalCategorySliderSettings} ref={slider}>
            {[...Array(15)]?.map((item, index) => {
              return <CustomRentalCard key={index} onlyshimmer />;
            })}
          </Slider>
        );
    }
  ...
  const rentalCategorySliderSettings = {
    dots: false,
    infinite: featuredCategories.length >= 7,
    speed: 500,
    slidesToShow: 7,
