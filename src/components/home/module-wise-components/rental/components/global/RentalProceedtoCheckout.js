...
const handleClick = () => {
  ...
  const getText = {
    cart: (
      <Typography sx={{ fontSize: 16, fontWeight: 400, display: "flex", gap: 1, color: (theme) => theme.palette.neutral[1000] }}>
        {rentalUserData?.user_data?.rental_type === "hourly" ? (
          <>
            {t("Estimated")} <b>{rentalUserData?.user_data?.estimated_hours}</b> Hrs
          </>
        ) : (
          <>
            {t("Estimated")} <b>{rentalUserData?.user_data?.distance?.toFixed(2)}</b> Km
          </>
        )}
      </Typography>
    ),
  ...
  return (
    <Box sx={sx}>
      {discountDifference && isShowDiscount ? (
        <Typography sx={{ fontSize: 14, fontWeight: 400, width: "100%", color: (theme) => theme.palette.neutral[1000], padding: "8px 0", backgroundColor: "#FFF6CA" }} align="center">
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 1 }}>
        {getText[textKey]}
        <Typography sx={{ fontSize: 22, fontWeight: 700 }}>{getAmountWithSign(totalAmount)}</Typography>
      </Box>
