...
const getData = async (pageParams) => {
  ...
  const { data } = await MainApi.get(`${categories_Childes_api}/${category_id}`);
  return data;
};
...
export default function PharmacyProductDynamicUI() {
  ...
  const handleScrollLeft = (catId) => {
    const el = scrollRefs.current[catId];
    if (el) el.scrollBy({ left: -(CARD_WIDTH + CARD_GAP) * 2, behavior: "smooth" });
  };
  ...
  /* ================= INCREMENT ================= */
  const handleIncrement = useCallback((cartItem) => {
    const userId = getUserIdentifier();
    ...
    dispatch(setIncrementToCartItem({ ...cartItem, quantity: newQty, totalPrice: newTotal, userId }));
  }, [dispatch]);

  /* ================= DECREMENT ================= */
  const handleDecrement = useCallback((cartItem) => {
    const userId = getUserIdentifier();
    const newQty = cartItem.quantity - 1;
    ...
    const newTotal = getUpdatedPrice(cartItem, newQty);
    dispatch(setDecrementToCartItem({ ...cartItem, quantity: newQty, totalPrice: newTotal, userId }));
  }, [dispatch]);
    ...
    return (
      <Box
        ...
        sx={{
          ...
          width: buttonWidth, height: "32px",
          border: "1.8px solid #16A34A", borderRadius: "6px",
        }}
      >
        <Box
        ...
        >
          <Typography sx={{ fontSize: "14px", color: isMaxReached ? "#aaa" : "#000" }}>+</Typography>
        </Box>
      </Box>
    );
  };
        ...
        [...Array(4)].map((_, i) => (
          <Box key={i} mb={6}>
            ...
            <Box sx={{ display: "flex", gap: 1.5, overflowX: "auto", pb: 1 }}>
              {[...Array(5)].map((_, j) => (
                <Skeleton key={j} variant="rounded" width={70} height={32} sx={{ borderRadius: "16px" }} />
              ))}
            </Box>
            <Box sx={{ display: "flex", gap: 1.5, overflowX: "auto", pb: 1 }}>
              {[...Array(4)].map((_, j) => (
                <Skeleton key={j} variant="rounded" width={180} height={230} sx={{ borderRadius: "8px" }} />
              ))}
            </Box>
          </Box>
        ))
        ...
        categories.map((cat) => {
          ...
          return (
            <Box key={cat.id} mb={6}>
              ...
              {/* SUBCATEGORIES */}
              <Box sx={{ display: "flex", gap: 1.5, overflowX: "auto", flexWrap: "nowrap", pb: 1 }}>
                {subcats.length === 0 ? (
                  [...Array(5)].map((_, i) => (
                    <Skeleton key={i} variant="rounded" width={70} height={32} sx={{ borderRadius: "16px" }} />
                  ))
                ) : (
                  subcats.map((sub) => (
                    <Chip
                      ...
                      sx={{
                        ...
                        backgroundColor: activeId === sub.id ? "#1A914B" : "transparent",
                        borderRadius: "8px",
                      }}
                    />
                  ))
              ...
              {!isProductLoading && productList.length === 0 ? (
                <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "180px", width: "100%" }}>
                  <Box component="img" src="/nomedicine.png" alt="No Medicines Found" title="No Medicines Found" sx={{ width: "130px", height: "130px", objectFit: "contain", mb: 1 }} />
                ...
                <Box sx={{ position: "relative", mt: "10px" }}>
                  ...
                  >
                    {(isProductLoading ? [...Array(5)] : productList).map((p, i) => {
                      // ── Skeleton card ──
                      if (isProductLoading) {
                        return (
                          <Card
                          ...
                          >
                            <Skeleton variant="rectangular" width="100%" height={130} animation="wave" sx={{ borderRadius: "8px" }} />
                            ...
                            <Box display="flex" justifyContent="space-between" mt={1}>
                              ...
                              <Skeleton variant="rectangular" width={50} height={28} animation="wave" sx={{ borderRadius: "6px" }} />
                            </Box>
                          </Card>
                        );
                      }
                      ...
                      // ── Real card ──
                      const name = p.name || p?.translations?.[0]?.value || "Unnamed Product";
                      ...
                      return (
                        ...
                        >
                          ...
                          <CardContent sx={{ p: 1.5 }}>
                            ...
                            <Typography variant="body2" fontWeight={600} noWrap sx={{ fontSize: "0.75rem", mb: 0.5 }}>
                              {name}
                            </Typography>
                            ...
                            <Box display="flex" justifyContent="space-between" alignItems="center" mt={0.5}>
                              <Box>
                                <Typography fontWeight="bold" sx={{ fontSize: "0.75rem", color: outOfStock ? "#999" : "#1A914B" }}>
                                  ₹{discountedPrice % 1 === 0 ? Math.floor(discountedPrice) : discountedPrice.toFixed(2)}
                                </Typography>
                                {discount > 0 && !outOfStock && (
                                  <Typography sx={{ textDecoration: "line-through", fontSize: "0.65rem" }}>
                                    ₹{p.price}
                                  </Typography>
                                )}
                              </Box>
