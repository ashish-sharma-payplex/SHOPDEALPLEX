...
// ─── Status color map ────────────────────────────────────────────────────────
const STATUS_STYLES = {
  ASSIGNED: { bg: "var(--ut-bg-d1fae5)", color: "var(--ut-text-065f46)" },
  REFUNDED: { bg: "var(--ut-bg-dcfce7)", color: "var(--ut-text-166534)" },
  REFUND_INITIATED: {
    bg: "var(--ut-bg-e0f2fe)",
    color: "var(--ut-text-0369a1)",
  },
  RESOLVED: { bg: "var(--ut-bg-fef9c3)", color: "var(--ut-text-854d0e)" },
  REJECTED: { bg: "var(--ut-bg-fee2e2)", color: "var(--ut-text-991b1b)" },
  CLOSED: { bg: "var(--ut-bg-f3f4f6)", color: "var(--ut-text-374151)" },
  PENDING: { bg: "var(--ut-bg-fff3e0)", color: "var(--ut-text-d97706)" },
};
      ...
      <CardContent sx={{ p: { xs: 1.5, sm: 2 } }}>
        ...
        {!isLoading && ticket && (
          <>
            ...
            {/* BBPS Remarks */}
            {/* {bbps && (
              <Box mt={2} sx={{ bgcolor: "var(--ut-bg-f0fdf4)", borderRadius: 2, p: 1.5 }}>
                <Typography fontSize={13} color="text.secondary" gutterBottom fontWeight={600}>
                  BBPS Remarks
                </Typography>
                <Typography fontSize={13} color="var(--ut-text-374151)">
                  {bbps.description || "—"}
                </Typography>
                {bbps.remarks && (
                  <Typography fontSize={12} color="var(--ut-text-6b7280)" mt={0.5}>
                    {bbps.remarks}
                  </Typography>
                )}
              </Box>
