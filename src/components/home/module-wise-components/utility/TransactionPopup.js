      ...
      {copyable && value && value !== "—" && !isPdfMode && (
        <Button
          ...
          sx={{
            ...
            flexShrink: 0,
            ml: 1,
            color: copied ? "var(--ut-text-2e7d32)" : "primary.main",
...
const TransactionPopup = ({ tx, onNeedHelp = () => {} }) => {
  ...
  const showRefund =
    tx.refund?.eligible === false && tx.refund?.refunded === true;
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const fileName = `transaction_${tx.status?.toLowerCase()}_${
        tx.client_request_id || Date.now()
      }.pdf`;
      pdf.save(fileName);
    } catch (err) {
  ...
  return (
    ...
    >
          ...
          {!isPdfMode && (
            ...
            >
              {downloading ? (
                ...
                <DownloadIcon
                  sx={{ fontSize: 20, color: "var(--ut-text-6b7280)" }}
                />
              )}
            </Box>
          )}
      ...
      <Box
        sx={{
          borderBottom: "1px dashed var(--ut-border-cccccc)",
          mx: 2,
          mb: 2,
        }}
      />
