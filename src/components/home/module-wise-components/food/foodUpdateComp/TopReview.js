...
import { Box, Typography, Button, Paper, IconButton, Skeleton } from '@mui/material';
...
const mapProductToOfferCardProps = (product) => {
    ...
    const ratingValue = product.avg_rating > 0 ? product.avg_rating.toFixed(1) : '0';
...
const OfferCard = ({ title, restaurant, deliveryTime, originalPrice, discountedPrice, rating, imageUrl, item, onCardClick, isLoading }) => {
    const { isWishlisted, addToWishlist, removeFromWishlist } = useWishlistHandler(item);
    ...
    return (
        ...
        >
            ...
            <Box sx={{ p: 1 }}>
                ...
                <Typography variant="body2" color="text.secondary" mb={1.2} sx={{ fontSize: '0.8rem' }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'baseline', pb: 0, mt: 0.5 }}>
                    <Typography variant="subtitle1" fontWeight="bold" color="text.primary" mr={1} lineHeight={1} sx={{ fontSize: '1rem' }}>
                        ₹{isLoading ? <Skeleton width={50} /> : Math.round(discountedPrice)}
                    </Typography>
                    {isLoading ? <Skeleton width={50} /> : originalPrice !== discountedPrice && (
                        <Typography
                            variant="body1"
...
const TopReviewedItemsLayout = () => {
    ...
    useEffect(() => {
        const fetchZoneId = async () => {
            try {
                ...
                const res = await fetch('https://dealplex.in/api/v1/items/most-reviewed', {
                    method: 'GET',
    ...
    useEffect(() => {
        const fetchOffers = async () => {
            try {
                ...
                const mappedOffers = (data.products || []).map(mapProductToOfferCardProps);

                if (mappedOffers.length === 0) throw new Error("API returned zero products.");
    ...
    if (loading) return <Box sx={{ p: 2, textAlign: 'center' }}><Typography>Loading top reviewed items...</Typography></Box>;
    if (offers.length === 0) return <Box sx={{ p: 2, textAlign: 'center' }}><Typography color="error">{error || "No items available."}</Typography></Box>;
    ...
    return (
        <Box sx={{ width: '100%', py: 2, boxSizing: 'border-box', margin: 0 }}>
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', mb: 2, px: 2 }}>
                <Box>
                    <Typography variant="h5" fontWeight="bold">Top Reviewed Items</Typography>
                    <Typography variant="subtitle1" color="text.secondary">Customer favorites with top ratings</Typography>
                </Box>
