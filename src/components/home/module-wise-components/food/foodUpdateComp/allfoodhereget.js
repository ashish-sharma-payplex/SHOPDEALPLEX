...
/* -------------------------
    ...
    ------------------------- */
const ProductCardDesignB = ({ product, handleProductPreview, loading }) => { // Added loading prop
    const name = product?.name || product?.translations?.[0]?.value || "Unnamed Product";
    const price = Number(product?.price) || 0;
    const discount = Number(product?.discount) || 0;
    const deliveryTime = product?.delivery_time || `${product?.min_delivery_time || 10}-${product?.max_delivery_time || 30} min`;
    const rating = product?.avg_rating > 0 ? Number(product.avg_rating).toFixed(1) : "0";
        ...
        >
            {loading ? (
                ...
                <Box>
                    ...
                    {/* Image Skeleton */}
                    <Skeleton variant="rectangular" width="100%" height={120} sx={{ pt: 1, mt: 2 }} />
                ...
                <>
                    ...
                    {discount > 0 && (
                        ...
                        >
                            ...
                            <Typography variant="caption" sx={{ lineHeight: 1, fontWeight: 700, fontSize: "0.6rem" }}>
                                OFF
                            </Typography>
                        </Box>
                    )}
                    ...
                    >
                        <img
                            ...
                            alt={name}
                            title={name}
                            style={{ width: "300px", maxHeight: "300px", objectFit: "contain", borderRadius: 8 }}
                            onClick={(e) => {
                                e.stopPropagation();
                  ...
                  <FavoriteBorderIcon sx={{ color: "var(--wishlist-inactive)", fontSize: 20 }} />
                    ...
                    <Box sx={{ p: 1, pt: 1.5 }}>
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
                            <AccessTimeIcon sx={{ fontSize: 14, color: GREEN_COLOR, mr: 0.5 }} />
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", pb: 1 }}>
                           <Box display="flex"alignItems="baseline"gap={0.6}>
                                <Typography variant="subtitle1"fontWeight="bold"color="text.primary"lineHeight={1}sx={{ fontSize:"1rem" }}>
...
/* -------------------------
    ...
    ------------------------- */
const ProductSliderDesignB = ({ products = [], pageLimit = 6, handleProductPreview, loading }) => { // Added loading prop
    ...
    const itemsToRender = loading ? Array.from({ length: skeletonCount }) : products.slice(0, pageLimit);
  ...
  useWishlistHandler(t);
    ...
    const fetchSubcategories = async (categoryId) => {
        try {
            ...
            const subcats = Array.isArray(data) ? data : data?.childes || data?.data?.childes || [];
            ...
            if (subcats.length > 0) {
                ...
                const prodData = await getSubCategoryProducts(firstSub.id, pageLimit, zoneIds);
    ...
    const isMainLoading = !initialLoadComplete || (initialLoadComplete && categories.length === 0);

    // Render basic loading text if we're waiting for zone Ids
    if (!initialLoadComplete) {
        return (
            <Container maxWidth="xl" sx={{ py: 4 }}>
                <Typography variant="h5" color="text.secondary">Loading location data...</Typography>
            </Container>
        );
    }
    ...
    return (
        <Container maxWidth="xl" sx={{ py: 4 }}>
            ...
            {categoriesToRender.map((cat, catIndex) => {
                ...
                const isSectionLoading = isMainLoading || (cat && (!subcats || subcats.length === 0));

                return (
                    <Box key={isMainLoading ? `section-skeleton-${catIndex}` : cat.id} mb={6}>
                        {/* Header */}
                        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2} px={2}>
                            {isSectionLoading ? (
                                <Skeleton variant="text" width="40%" height={36} />
                            ) : (
                                <Typography variant="h4" fontWeight={600} sx={{ fontSize: "1.5rem" }}>
                                    {cat.name}
                                </Typography>
                            )}
                        ...
                        >
                            {isSectionLoading ? (
                                ...
                                Array.from({ length: 4 }).map((_, i) => (
                                    <Skeleton key={i} variant="rectangular" width={80} height={32} sx={{ borderRadius: 8, flexShrink: 0 }} />
                                ))
                            ) : (
                                // Actual Chips
                                subcats.map((sub) => (
