...
const postData = async (storeData) => {
  const translationsR = [];
  if (storeData.restaurant_name && typeof storeData.restaurant_name === 'object') {
    for (const [locale, name] of Object.entries(storeData.restaurant_name)) {
      translationsR.push({ id: null, locale, key: "name", value: name });
    }
  }

  if (storeData.restaurant_address && typeof storeData.restaurant_address === 'object') {
    for (const [locale, address] of Object.entries(storeData.restaurant_address)) {
